...
def record_llm_call(
    ...
    def get_stats(self, since: Optional[datetime] = None) -> LLMUsageStats:
        """
        Get aggregated usage statistics.

        Args:
            since: Only include calls after this time (default: all time)

        Returns:
            LLMUsageStats with aggregated metrics
        """
        stats = LLMUsageStats()
        stats.by_model = defaultdict(
            lambda: {"calls": 0, "tokens": 0, "cost": 0.0, "latency_ms": 0.0}
        )
        stats.by_agent = defaultdict(
            lambda: {"calls": 0, "tokens": 0, "cost": 0.0, "latency_ms": 0.0}
        )
        stats.by_operation = defaultdict(
            lambda: {"calls": 0, "tokens": 0, "cost": 0.0, "latency_ms": 0.0}
        )
        ...
        for call in calls:
            ...
            stats.by_model[call.model]["tokens"] += call.total_tokens
            stats.by_model[call.model]["cost"] += call.total_cost
            stats.by_model[call.model]["latency_ms"] += call.latency_ms

            # By agent
            stats.by_agent[call.agent_name]["calls"] += 1
            stats.by_agent[call.agent_name]["tokens"] += call.total_tokens
            stats.by_agent[call.agent_name]["cost"] += call.total_cost
            stats.by_agent[call.agent_name]["latency_ms"] += call.latency_ms

            # By operation
            stats.by_operation[call.operation]["calls"] += 1
            stats.by_operation[call.operation]["tokens"] += call.total_tokens
            stats.by_operation[call.operation]["cost"] += call.total_cost
            stats.by_operation[call.operation]["latency_ms"] += call.latency_ms

        # Convert defaultdicts to regular dicts and calculate averages
        for breakdown in [stats.by_model, stats.by_agent, stats.by_operation]:
            for key in breakdown:
                if breakdown[key]["calls"] > 0:
                    breakdown[key]["avg_latency_ms"] = round(
                        breakdown[key]["latency_ms"] / breakdown[key]["calls"], 2
                    )
                    breakdown[key]["cost"] = round(breakdown[key]["cost"], 4)
                del breakdown[key]["latency_ms"]  # Remove raw total
